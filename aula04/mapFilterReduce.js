...
// 1 - Pegar TODAS os títulos
const titulos = dadosTreino.map(function (item) {
  return item.title;
});

// 2 - Pegar TODAS as descrições das tarefas COMPLETAS
function ehCompleto(item) {
  if (item.completed) {
    return true;
  } else {
    return false;
...
function pegaTarefaTitulo(item) {
  return item.title;
}

const apenasCompletas = dadosTreino.filter(ehCompleto).map(pegaTarefaTitulo);

// 3 - Pegar a quantidade de tarefas completas
// const totalDeConcluidos = dadosTreino.reduce(function (acumulador, item) {
//   if(item.completed) {
//     return acumulador.push(item)
//   } else{
//     return acumulador
//   }
// }, []);
...
const novosItens = produtos.reduce(function (acumulador, item) {
  return acumulador + item.valor;
}, 0);

console.log(novosItens);

// 4 - Alterar TODAS as tarefas para que a descrição seja "Hello Cacto"
